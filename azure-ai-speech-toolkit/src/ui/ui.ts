...
//   localizer: Localizer;
  ...
  async selectOption(config: SingleSelectConfig): Promise<Result<SingleSelectResult, Error>> {
    ...
    try {
      ...
      return await new Promise<Result<SingleSelectResult, Error>>((resolve) => {
  ...
  async selectOptions(config: MultiSelectConfig): Promise<Result<MultiSelectResult, Error>> {
    ...
    try {
      ...
      quickPick.placeholder = config.placeholder
        ? config.placeholder + " (Space key to check/uncheck)"
        : " (Space key to check/uncheck)";
      quickPick.ignoreFocusOut = true;
      quickPick.matchOnDescription = true;
      quickPick.matchOnDetail = true;
      quickPick.canSelectMany = true;
      const preIds: Set<string> = new Set<string>();
      return await new Promise<Result<MultiSelectResult, Error>>((resolve) => {
        let options: StaticOptions = [];
        let isSkip = false;
        ...
        if (typeof config.options === "function" || typeof config.default === "function") {
          ...
          Promise.race([loadDynamicData(), timeoutPromise])
            ...
            .catch((e) => resolve(new Err(new Error(e))));
        } else {
          options = config.options;
  ...
  async inputText(config: InputTextConfig): Promise<Result<InputTextResult, Error>> {
    const disposables: Disposable[] = [];
    try {
      ...
      return await new Promise<Result<InputTextResult, Error>>((resolve): void => {
        let defaultValue: string | undefined = undefined;
        const loadDynamicData = async () => {
          try {
            if (typeof config.default === "function") {
        ...
        if (typeof config.default === "function") {
          inputBox.busy = true;
          inputBox.enabled = false;
          inputBox.placeholder = "Loading default value ...";
          loadDynamicData()
            .then(onDataLoaded)
            .catch((e) => resolve(new Err(new Error(e))));
        } else {
  ...
  async selectFolder(config: SelectFolderConfig): Promise<Result<SelectFolderResult, Error>> {
    const disposables: Disposable[] = [];
    try {
      const quickPick = window.createQuickPick<FxQuickPickItem>();
      quickPick.title = config.title;
      ...
      quickPick.matchOnDetail = true;
      quickPick.canSelectMany = false;
      return await new Promise<Result<SelectFolderResult, Error>>((resolve) => {
        let defaultValue: string | undefined = undefined;
        const loadDynamicData = async () => {
          try {
            if (typeof config.default === "function") {
        ...
        if (typeof config.default === "function") {
          quickPick.busy = true;
          quickPick.placeholder = "Loading default value ...";
          loadDynamicData()
            .then(onDataLoaded)
            .catch((e) => resolve(new Err(new Error(e))));
        } else {
  ...
  async selectFile(config: SelectFileConfig): Promise<Result<SelectFileResult, Error>> {
  ...
  async selectFiles(config: SelectFilesConfig): Promise<Result<SelectFilesResult, Error>> {
    if (config.default && typeof config.default === "function") {
      //TODO  quick workaround solution, which will blocking the UI popup
      config.default = await config.default();
    }
  ...
  async selectFileInQuickPick(
    ...
    type: "file" | "files",
    defaultValue?: string
  ): Promise<Result<SelectFileResult, Error>>;
  async selectFileInQuickPick(
    config: SelectFilesConfig,
    type: "file" | "files",
    defaultValue?: string
  ): Promise<Result<SelectFilesResult, Error>>;
  async selectFileInQuickPick(
    config: UIConfig<any> & {
    ...
    type: "file" | "files",
    defaultValue?: string
  ): Promise<Result<InputResult<string[] | string>, Error>> {
  ...
  async openUrl(link: string): Promise<Result<boolean, Error>> {
  ...
  ): Promise<Result<InputResult<string>, Error>> {
      ...
      if (selectFileOrItemRes.isOk()) {
        if (selectFileOrItemRes.value.result === config.inputOptionItem.id) {
          ...
          if (inputRes.isOk()) {
            ...
            return new Err(inputRes.error);
          }
        } else {
          return ok(selectFileOrItemRes.value);
        }
      } else {
        return new Err(selectFileOrItemRes.error);
      }
    }
  }

  async confirm(config: ConfirmConfig): Promise<Result<ConfirmResult, Error>> {
      ...
      try {
        ...
        promise.then(
          ...
          (error) => {}
                ...
                resolve(new Err(error as Error));
                // resolve(error);
      }
    });
  }
  ...
  //       clearTimeout(wait);
  //       reject(
  //           new Error("Execute command timeout: " + cmd)
  ...
  public async openFile(filePath: string): Promise<Result<boolean, Error>> {
