...
export class AzureAccountNode extends DynamicNode {
  ...
  public override getTreeItem(): vscode.TreeItem | Promise<vscode.TreeItem> {
    ...
    if (this.status === AccountItemStatus.SignedIn) {
      ...
      this.label = "Azure: Signing in...";
      // localize("teamstoolkit.accountTree.signingInAzure");
    } else {
      this.label = "Sign in to Azure";
      // localize("teamstoolkit.handlers.signInAzure");
      this.collapsibleState = vscode.TreeItemCollapsibleState.None;
      // this.command = {
      //   title: this.label,
    ...
    this.tooltip = new vscode.MarkdownString(
      "AZURE ACCOUNT  \nThe Teams Toolkit requires Azure subscription to deploy the Azure resources for your project."
